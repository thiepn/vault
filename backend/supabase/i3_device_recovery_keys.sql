...
begin
  ...
  if p_confirmation !~ '^[A-Za-z0-9_-]{43}
  p_vault_id uuid,
  p_device_id uuid,
  p_active_generation integer,
  p_envelopes jsonb,
...
grant execute on function public.vault_key_rotate_recovery(uuid,uuid,jsonb) to authenticated;
 then
  ...
  for update;
  if not found
    or v_request.key_generation is null
    or v_request.key_generation<>v_state.active_generation then
    raise exception 'Device approval is stale after Vault key rotation' using errcode='40001';
  end if;
  ...
  if not exists(
    ...
    where e.vault_id=v_request.vault_id
      and e.account_id=v_account
      and e.device_id=p_device_id
      and e.key_generation=v_state.active_generation
  ) or not exists(
    select 1
    from vault_private.recovery_vault_key_envelopes r
    where r.vault_id=v_request.vault_id
      and r.account_id=v_account
      and r.key_generation=v_state.active_generation
  ) then
    raise exception 'Active Vault-key readiness is incomplete';
  end if;

  insert into vault_private.device_vault_access(
    vault_id,account_id,device_id,authorized_at,revoked_at
  ) values(
    v_request.vault_id,v_account,p_device_id,now(),null
  )
  on conflict(vault_id,account_id,device_id) do update
    set authorized_at=excluded.authorized_at,revoked_at=null;
  ...
  return vault_private.readiness_json(
    v_request.vault_id,v_account,p_device_id
  );
end;
$;
