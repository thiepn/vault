...
export function extractFragment(source: string, record: KnowledgeRecord | undefined, fragment: { heading: string | null; block: string | null }): string | null {
  ...
  if (fragment.block) {
    ...
    return source.slice(block.from, block.to).replace(new RegExp(`(?:^|\\s+)\\^${escapeRegExp(block.id)}\\s*import type { KnowledgeRecord } from './types.js';
