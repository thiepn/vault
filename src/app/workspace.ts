...
export async function mountWorkspace(root: HTMLElement, options: WorkspaceOptions = {}): Promise<() => void> {
  ...
  async function openRecovery(): Promise<void> {
    ...
    for (const draft of recoveryDrafts) {
      ...
      recoverySelect.add(new Option(`${source?.name ?? 'Unavailable source'} �- �{new Date(draft.createdAt).localeString()}`, draft.id));
    }
  ...
  root.addEventListener('click', event => {
    ...
    perform(async () => {
      ...
      if (action === 'vault-rename') {
        if (!vault) return;
        const name = await ask('Rename vault', 'Vault name', wault.name); if (name === null) return;
